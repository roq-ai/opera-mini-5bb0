...
import { getWalletBalances } from 'apiSdk/wallet-balances';
...
import { NotificationSettingsListPage as ShowCaseList } from 'pages/notification-settings';
...
function DashboardPage() {
  ...
  const walletBalanceStat = useSWR<PaginatedInterface<WalletBalanceInterface>>(
    () => `/wallet-balances`,
    () => getWalletBalances({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: walletBalanceStat?.data?.totalCount,
      isLoading: walletBalanceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Notification Settings
                </Text>
                <NextLink href={`notification-settings`} passHref legacyBehavior>
                  <Button
